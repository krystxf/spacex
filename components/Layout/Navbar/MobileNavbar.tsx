    ...
    <Menu as="div" className="block md:hidden">
      ...
      >
        <Menu.Items className="w-full max-w-[250px] absolute top-0 right-0 h-screen p-10 bg-black">
          <div
            className="absolute z-30 right-20 top-20 flex flex-col justify-between"
...
const MobileNavItem: React.FC<{ link: string; title: string }> = ({
  link,
  title
}) => {
  return (
    <Link href={link}>
      <a className="text-gray-200 hover:text-white py-5 transition-all duration-500 ease-in-out">
        {title.toUpperCase()}
      </a>
    </Link>
  )

...
const Mission: NextPage<MissionPageProps> = ({ mission }) => {
    ...
    return <Layout pathname="/missions" >
        <StarmanScreen title={`mission ${mission.name}`} backgroundImage='/FH_2.jpg'>
            <>
                <motion.div
                ...
                >
                    ...
                    {mission.payloads && // only show when there are any payloads
                        <div className="pt-4">
                            ...
                            <h2 className="text-2xl font-bold">{mission.payloads?.length > 1 ? "PAYLOADS" : "PAYLOAD"}:</h2>
                            total of {mission.payloads.length} payloads

                            {mission.payloads.map((payload, index) => payload // some payloads can be null 
                                && <div className={`flex my-10`}>
                                    ...
                                    >
                                        ...
                                        <div className="flex flex-col text-white font-medium divide-y divide-gray-500 w-full ">
                                            <Row title="manufacturer" value={payload?.manufacturer ?? ""} />
                                            <Row title="nationality" value={payload?.nationality ?? ""} />
                                            <Row title="type" value={payload?.payload_type ?? ""} />
